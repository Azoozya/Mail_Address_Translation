...
// Generate address : gen_address GET
// Request for an address (submit client/domain) new_address POST
#[get("/new_address")]
pub async fn get_new_address() -> Result<NamedFile, NotFound<String>> {
    NamedFile::open("static/forms/new_address.html")
        .await
        .map_err(|e| NotFound(e.to_string()))
}

#[post("/new_address", data = "<args_address>")]
pub async fn post_new_address(mut args_address: Form<Address>) -> Result<String, Status> {
    if let Err(e) = args_address.clean() {
    ...
    let mut dmn = MATRow::Domain{
        id: 0,
        name: args_address.domain.domain(),
        nb_ref: 0,
    };
